package com.hrms.business.salary.service.impl;

import com.hrms.business.salary.dto.SalaryMonthlyQueryDTO;
import com.hrms.business.salary.service.SalaryService;
import com.hrms.business.salary.vo.SalaryMonthlyVO;
import com.hrms.common.model.PageResult;
import java.math.BigDecimal;
import org.springframework.stereotype.Service;

/**
 * 实现薪资业务接口能力。
 */
@Service
public class SalaryServiceImpl implements SalaryService {

    /**
     * 分页查询月度薪资。
     *
     * @param queryParam 月度薪资查询参数
     * @return 月度薪资分页结果
     */
    @Override
    public PageResult<SalaryMonthlyVO> pageMonthlySalary(SalaryMonthlyQueryDTO queryParam) {
        return PageResult.empty(queryParam.getPageNum(), queryParam.getPageSize());
    }

    /**
     * 查询月度薪资详情。
     *
     * @param id 月度薪资ID
     * @return 月度薪资详情
     */
    @Override
    public SalaryMonthlyVO getMonthlySalary(Long id) {
        return new SalaryMonthlyVO(id, 1L, "2026-07", BigDecimal.ZERO, BigDecimal.ZERO, "DRAFT");
    }
}
